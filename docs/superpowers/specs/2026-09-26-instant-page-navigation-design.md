...
**หลัง deploy (2026-09-26, เว็บจริง, เปิดหน้าครั้งที่สอง):** เนื้อหาหลักอยู่บนจอแล้วที่ 141–148ms ทุกหน้า (หน้าหลัก, ตะลุยด่าน, คลังคำศัพท์, Cafe, โปรไฟล์) ขณะที่ `accounts:lookup` ยังไม่ตอบ; เปิด `/pik-a-class/` ถึงหน้าหลักที่ 151ms (ข้าม login) เส้นทางด่านวัดด้วยเครื่องมือไม่ได้ (extension บล็อกหน้าที่มี query string) — ยังไม่ได้วัดบนมือถือจริง
